...
const ClassRoom = () => {
  const activeTab = useSelector((state) => state?.tab?.status);
    ...
    <>
      <div>
        ...
        <div className={styles.tabWrap}>
          <Tab1 data={tabData} activeTab={activeTab} />
        </div>
...
export const ClassworkViewWrapper = () => {
  ...
  const { sendRequest } = useDeleteClasswork(mutate);

  // Delete module API request hook
  const { deleteModule } = useDeleteModule(mutate);
